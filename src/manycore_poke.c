...
void main(uint64_t argc, char * argv[]) {
    ...
    // Construct EVA
    uint32_t *mc_csr_pc_eva = (uint32_t *) (bp_coproc_offset | dram_not_tile | tile_not_dram | tile_y_cord | tile_x_cord | mc_csr_pc_epa);
    uint32_t *mc_csr_freeze_eva = (uint32_t *) (bp_coproc_offset | dram_not_tile | tile_not_dram | tile_y_cord | tile_x_cord | mc_csr_freeze_epa);
